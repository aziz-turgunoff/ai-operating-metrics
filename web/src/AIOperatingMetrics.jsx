...
function statusMeta(status) {
  ...
  if (status === "error") return { dot: T.error, bg: "rgba(224,91,91,0.08)", text: "ERROR" };
...
function NorthStarCard({ m, src }) {
  const meta = statusMeta(src?.status ?? "mock");
  ...
  return (
    ...
    >
      ...
      <div style={{ fontFamily: T.mono, fontSize: 26, fontWeight: 600, color: empty ? T.inkFaint : T.ink, letterSpacing: -0.5 }}>
        {fmt(m.value, m.unit)}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 10 }}>
        ...
        <span style={{ fontFamily: T.mono, fontSize: 10, color: T.inkFaint }}>{src?.label ?? m.source}</span>
      </div>
      ...
      {open && (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${T.panelEdge}` }}>
          ...
          {Array.isArray(history) && history.length >= 2 && <Sparkline points={history} />}
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <span style={{ fontFamily: T.mono, fontSize: 15, color: empty ? T.inkFaint : T.ink, minWidth: 64, textAlign: "right" }}>
          {fmt(m.value, m.unit)}
        </span>
        <span title={src?.label ?? m.source} style={{ width: 7, height: 7, borderRadius: 99, background: meta.dot, flexShrink: 0 }} />
      </div>
    </div>
  );
}
...
export default function AIOperatingMetrics() {
  ...
  const sourceOf = (key) => ({ ...sourcesMeta[key], status: sources[key]?.status ?? "mock" });
  ...
  if (err) {
    return (
      <div style={{ background: T.bg, minHeight: "100vh", color: T.error, fontFamily: T.mono, padding: 32 }}>
        Failed to load /api/metrics — is the server running on :8787? ({err})
      </div>
    );
  }
  if (!payload) {
    return <div style={{ background: T.bg, minHeight: "100vh", color: T.inkDim, fontFamily: T.mono, padding: 32 }}>Loading…</div>;
  }

  return (
    <div style={{ background: T.bg, minHeight: "100vh", fontFamily: T.sans, color: T.ink, padding: "0 0 64px" }}>
      <header style={{ padding: "36px 32px 24px", borderBottom: `1px solid ${T.panelEdge}` }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap" }}>
            ...
            <span style={{ fontFamily: T.mono, fontSize: 12, color: T.inkDim }}>
          ...
          <button onClick={takeSnapshot} style={{
            ...
            borderRadius: 8, padding: "7px 13px", fontSize: 12, fontFamily: T.mono, cursor: "pointer",
          }}>
            Save snapshot now
          </button>
        </div>
        ...
        {snapshotMsg && <p style={{ margin: "0 0 14px", color: T.accent, fontSize: 11.5, fontFamily: T.mono }}>{snapshotMsg}</p>}
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {[
            ...
            <button key={b.k} onClick={() => setFilter(b.k)} style={{
              ...
              borderRadius: 8, padding: "7px 13px", fontSize: 12.5, fontFamily: T.mono,
            }}>
      ...
      <section style={{ padding: "26px 32px 8px" }}>
        <div style={{ fontFamily: T.mono, fontSize: 11, color: T.accent, letterSpacing: 1, marginBottom: 14 }}>
          NORTH STAR · click a card for its trend
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 12 }}>
          {report.northStar.map((m) => <NorthStarCard key={m.key} m={m} src={sourceOf(m.source)} />)}
        </div>
      </section>
        ...
        {report.categories.filter((c) => showCat(c.metrics)).map((cat) => {
          ...
          return (
            <div key={cat.name} style={{ background: T.panel, border: `1px solid ${T.panelEdge}`, borderRadius: 12, overflow: "hidden" }}>
              ...
              {visible.map((m, i) => <MetricRow key={i} m={m} src={sourceOf(m.source)} />)}
            </div>
          );
        })}
      </section>

      <footer style={{ padding: "32px 32px 0", color: T.inkFaint, fontSize: 11.5, fontFamily: T.mono }}>
