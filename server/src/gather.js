...
import { companyai } from "./connectors/companyai.js";
...
export async function gatherSources() {
  const [or_, ow, lb, ap, qd, ff, ci, wf, ca] = await Promise.all([
    openrouter(), openwebui(), leadbank(), apollo(), qdrant(), fireflies(), cicd(), workflows(), companyai(),
  ]);
  const derivedResult = derived({ workflowsResult: wf });

  return {
    openrouter: or_, openwebui: ow, leadbank: lb, apollo: ap,
    qdrant: qd, fireflies: ff, cicd: ci, workflows: wf, companyai: ca,
